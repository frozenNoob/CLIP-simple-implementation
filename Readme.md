...
(2) 不太推荐： 直接租显存更高的服务器，但是成本较高。根据查阅到的资料：OpenAI公司对预训练阶段的CLIP用了大量的训练数据以及训练资源，大力出奇迹。CLIP用了400million的**image-text pair**对进行训练，对于image backbone，CLIP尝试了两种结构，DN50x64 和 vit-L，分别用了592 个 V100 + 18天 的时间 和 256 个 V100 + 12天 的时间，非大公司直接劝退。

(3) **推荐**：选取一个大小和步长都为  $(batch\ size, batch\ size)$ 的滑动窗口 来遍历**图-文对**矩阵中的元素（实际上实现时，可以采用**惰性加载**的方式以此避免直接计算较大的**图-文对**矩阵），以此在低成本的情况下避免显存占用过高，同时也能遍历所有的**图-文对**。当然，这可能会导致反向传播过程“不够充分”（因为这相当于把 $loss$ 拆分成 $batch\ size$ 份并把每一份进行一次反向传播以更新参数，显然不符合 $InfoNCE\ Loss$  的公式）。
